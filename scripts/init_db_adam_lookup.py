"""Initialize (drop + recreate) the storms.adam_fm_lookup table.
...
rather than an in-place migration: adam_fm_lookup.sql now DROPs then
recreates the typed, constrained table. After running this, repopulate
with `python scripts/build_adam_fm_lookup.py --write-db --mode <mode>`.
...
Usage
-----
    python scripts/init_db_adam_lookup.py --mode dev
"""
...
def main() -> None:
    ...
    with engine.connect() as conn:
        ...
        conn.commit()
        print(f"Initialized adam_fm_lookup table for mode: {args.mode}")
