"""
Calculate population exposure to IBTrACS wind buffer polygons at ADM0 level.

Reads wind buffer polygons from storms.ibtracs_wind_buffers (dev DB) and
calculates how many people in each country fall within each buffer using
the WorldPop 2026 1km raster. Results are written to storms.ibtracs_wind_exposure
(admin_level=0 rows).

Usage:
    python scripts/calc_ibtracs_adm0_exp.py                       # all countries, all storms
    python scripts/calc_ibtracs_adm0_exp.py --test                 # CUB, NA basin, since 2020
    python scripts/calc_ibtracs_adm0_exp.py --countries CUB HTI MEX
    python scripts/calc_ibtracs_adm0_exp.py --since 2020 --basin NA
    python scripts/calc_ibtracs_adm0_exp.py --overwrite            # recalculate everything
"""
...
KEY_COLS = ["sid", "wind_speed_kt", "admin_level", "pcode"]
ADMIN_LEVEL = 0
TEST_COUNTRIES = ["CUB"]
TEST_SINCE = 2020
TEST_BASIN = "NA"
...
def load_buffers(engine, since: int | None = None, basin: str | None = None) -> gpd.GeoDataFrame:
    filters = []
    if since:
    ...
    desc = []
    if since:
        desc.append(f"since season {since}")
    if basin:
        desc.append(f"basin={basin}")
    label = f" ({', '.join(desc)})" if desc else ""
    print(f"Loading IBTrACS wind buffers from DB{label}...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Calculate IBTrACS wind buffer ADM0 population exposure")
    group = parser.add_mutually_exclusive_group()
    group.add_argument("--test", action="store_true",
                       help=f"Run for {', '.join(TEST_COUNTRIES)}, {TEST_BASIN} basin, since season {TEST_SINCE}")
    group.add_argument("--countries", nargs="+", metavar="ISO3", help="Specific country ISO3 codes to process")
    parser.add_argument("--since", type=int, metavar="YEAR", help="Only include storms from this season year onwards")
