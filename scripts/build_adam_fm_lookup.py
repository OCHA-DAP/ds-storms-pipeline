...
`scripts/_archive/build_adam_fm_lookup.py`), which drove the lookup from
...
def main() -> int:
    ...
    if args.write_db and not args.skip_validation:
        blocking = validate(args.humrev, "adam")
        if blocking:
            logger.error(
    ...
    if args.write_db:
        ...
        # adam_admin_id is a nullable integer; keep it as pandas Int64 so it
        # appends cleanly into the BIGINT column (no float coercion).
        df["adam_admin_id"] = df["adam_admin_id"].astype("Int64")

        # Full rebuild into the PRE-CREATED typed/constrained table (run
        # scripts/init_db_adam_lookup.py first). TRUNCATE + append in one
        # transaction: atomic (no visible-empty window) and preserves the
        # DDL types/UNIQUE constraint — unlike if_exists="replace", which
        # drops the table and re-infers dtypes (regressing adam_admin_id to
        # float and dropping the constraint).
